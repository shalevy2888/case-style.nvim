...
local gui = require('case-style.gui')
...
local function _word_under_cusror()
    -- Get row and column
    local row, column
    if table.unpack == nil then ---@diagnostic disable-line
        row, column = unpack(vim.api.nvim_win_get_cursor(0))
    else
        row, column = table.unpack(vim.api.nvim_win_get_cursor(0)) ---@diagnostic disable-line
    end
...
local function _project_rename(new_name, open_qflist)
    ...
    vim.lsp.buf_request(0, "textDocument/rename", position_params, function(err, result, ...)
        ...
        if true then
            P(err) ---@diagnostic disable-line
            P(result) ---@diagnostic disable-line
        end
        if result == nil then
            return
        end
...
local function _buffer_rename(old_name, new_name, open_qflist)
    ...
    local entries = {}
    local new_name_len = string.len(new_name)
    for index, line in pairs(buf) do
        local i, j = nil, nil
        while true do
            i, j = string.find(line, old_name, i)
...
M.change_case_with_options = function(fargs_table)
    P(fargs_table)
    local text_to_replace = fargs_table[1]
    local new_text = fargs_table[2]
    local scope = fargs_table[3]
    ...
    if scope == 'single' then
        ...
        print(new_text)
        _replace_text(start_row, start_col, end_row, end_col, new_text)
    elseif scope == 'buf' then
        -- print('qflist value in config: ', config.get_mod_config('qflist'))
        _buffer_rename(text_to_replace, new_text, config.get_mod_config('qflist'))
    elseif scope == 'project' then
        -- print('project rename')
        _project_rename(new_text, config.get_mod_config('qflist'))
    else
...
M.change_case = function(style, s)
    ...
    local current_word, start_row, start_col, end_row, end_col
    local style_words_table = {}
    if cur_mode == 'n' then
        current_word, start_row, start_col, end_row, end_col = _word_under_cusror()
        -- print(current_word)
        if current_word == nil or current_word == '' then
            return
        end
        ...
        style_words_table = {
            _snake_case(divided_words), _camel_case(divided_words), _pascal_case(divided_words),
            _kebab_case(divided_words), _caps_case(divided_words),
        }
    end
    if style == 'window' then
        if cur_mode == 'n' then
            gui.open_input_and_options_windows(current_word, style_words_table)
        end
        return
    end
...
vim.keymap.set("n", "<leader>cw", ":CaseStyle window<CR>")
...
vim.api.nvim_create_user_command(
    'CaseStyleReplace',
    function(opts)
        M.change_case_with_options(opts.fargs)
    end, -- replace what, withwhat, scope
    { nargs = '+' }
)
